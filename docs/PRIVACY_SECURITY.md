...
- `audit_log` is append-only (no UPDATE/DELETE exposed through the app) and
  records every ingestion, edit, export, and OCR run.
- Every generated binder is recorded in `binder_exports` with a hash of the
  output file and the exact set of documents/events included, so a binder
